...
// Get each of the 4 rays in the corners of the camera's frustum
...
void renderScene()
{
	...
	calcCameraRays(cameraPos, glm::vec3(0.0f, 0.5f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), 45.0f, (float)renderW / renderH);

	glBindImageTexture(0, tex_output, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA8);
	glDispatchCompute((GLuint)renderW, (GLuint)renderH, 1);
	...
	FIBITMAP* image = FreeImage_ConvertFromRawBits(
		pixels, renderW, renderH, 3 * renderW, 24, 0xFF0000, 0x00FF00, 0x0000FF, false);
...
void init()
{
	...
	glUseProgram(ray_program);
	eye_loc = glGetUniformLocation(ray_program, "eye");
	ray00 = glGetUniformLocation(ray_program, "ray00");
	ray01 = glGetUniformLocation(ray_program, "ray01");
	ray10 = glGetUniformLocation(ray_program, "ray10");
	ray11 = glGetUniformLocation(ray_program, "ray11");
	...
	if (useWindow)
	{
		...
		glAttachShader(quad_program, vertex_shader);		// This attaches our vertex shader to our program.
		glAttachShader(quad_program, fragment_shader);	// This attaches our fragment shader to our program.
...
int main(int argc, char **argv)
{
	...
	if(useWindow)
		window = glfwCreateWindow(screenW, screenH, "", nullptr, nullptr);

	else
		window = glfwCreateWindow(400, 1, "", nullptr, nullptr);
	...
	// Make the BYTE array, factor of 3 because it's RGB.
	// This will hold each screenshot
	pixels = new unsigned char[3 * renderW * renderH];
