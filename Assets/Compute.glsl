...
#define NUM_TRIANGLES 58
#define MAX_LIGHTS 5

const light lights[] = light[5](
	//pos, color, radius, brightness

	// red light
	light(vec3(5.0, 5.0, 5.0), vec3(1.0, 0.0, 0.0), 10.0, 1.0),

	// green light
	light(vec3(-3.0, 2.0, 5.0), vec3(0.0, 1.0, 0.0), 5.0, 1.0),

	// blue light
	light(vec3(5.0, 2.0, -5.0), vec3(0.0, 0.0, 1.0), 5.0, 3.0),

	// white light
	light(vec3(-1.0, 4.0, -1.0), vec3(1.0, 1.0, 1.0), 5.0, 6.0),

	// orange light
	light(vec3(-4.0, 0.5, -4.0), vec3(1.0, 0.25, 0.1), 2.0, 2.0)
);
...
const triangle triangles[] = triangle[58](
	...
	// -X Wall
	triangle(vec3(-6.0, 5.0, 5.0), vec3(-6.0, 5.0, -5.0), vec3(-6.0, 0.0, -5.0), vec3(1.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0)),
	triangle(vec3(-6.0, 5.0, 5.0), vec3(-6.0, 0.0, -5.0), vec3(-6.0, 0.0, 5.0), vec3(1.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0)),
	...
	// -Z Wall
	triangle(vec3(5.0, 5.0, -6.0), vec3(-5.0, 5.0, -6.0), vec3(-5.0, 0.0, -6.0), vec3(0.0, 0.0, 1.0), vec3(1.0, 1.0, 1.0)),
	triangle(vec3(5.0, 5.0, -6.0), vec3(-5.0, 0.0, -6.0), vec3(5.0, 0.0, -6.0), vec3(0.0, 0.0, 1.0), vec3(1.0, 1.0, 1.0)),
	...
	/* Cube Box */
	/* Back face triangles */
	triangle(vec3(-0.5, 1.0, -2.5), vec3(0.5, 1.0, -2.5), vec3(-0.5, 2.0, -2.5), vec3(0.0, 0.0, -1.0), vec3(1.0, 0.5, 0.1)),
	triangle(vec3(0.5, 1.0, -2.5), vec3(0.5, 2.0, -2.5), vec3(-0.5, 2.0, -2.5), vec3(0.0, 0.0, -1.0), vec3(1.0, 0.5, 0.1)),
	...
	triangle(vec3(0.5, 1.0, 0.5), vec3(0.5, 2.0, 0.5), vec3(0.5, 2.0, -2.5), vec3(1.0, 0.0, 0.0), vec3(1.0, 0.5, 0.1)),
	triangle(vec3(0.5, 1.0, 0.5), vec3(0.5, 2.0, -2.5), vec3(0.5, 1.0, -2.5), vec3(1.0, 0.0, 0.0), vec3(1.0, 0.5, 0.1)),
	/* Left face triangles */
	triangle(vec3(-0.5, 1.0, -2.5), vec3(-0.5, 2.0, -2.5), vec3(-0.5, 2.0, 0.5), vec3(-1.0, 0.0, 0.0), vec3(1.0, 0.5, 0.1)),
	triangle(vec3(-0.5, 1.0, -2.5), vec3(-0.5, 2.0, 0.5), vec3(-0.5, 1.0, 0.5), vec3(-1.0, 0.0, 0.0), vec3(1.0, 0.5, 0.1)),
	/* Top face triangles */
	triangle(vec3(-0.5, 2.0, 0.5), vec3(-0.5, 2.0, -2.5), vec3(0.5, 2.0, -2.5), vec3(0.0, 1.0, 0.0), vec3(1.0, 0.5, 0.1)),
	triangle(vec3(-0.5, 2.0, 0.5), vec3(0.5, 2.0, -2.5), vec3(0.5, 2.0, 0.5), vec3(0.0, 1.0, 0.0), vec3(1.0, 0.5, 0.1)),
	/* Bottom face triangles */
	triangle(vec3(-0.5, 1.0, 0.5), vec3(0.5, 1.0, 0.5), vec3(0.5, 1.0, -2.5), vec3(0.0, -1.0, 0.0), vec3(1.0, 0.5, 0.1)),
	triangle(vec3(-0.5, 1.0, 0.5), vec3(0.5, 1.0, -2.5), vec3(-0.5, 1.0, -2.5), vec3(0.0, -1.0, 0.0), vec3(1.0, 0.5, 0.1)),

	// Blue box
	/* Cube Box 2 */
	/* Back face triangles */
	triangle(vec3(2.5, 1.5, 2.5), vec3(3.5, 1.5, 2.5), vec3(2.5, 4.5, 2.5), vec3(0.0, 0.0, 1.0), vec3(0.1, 0.5, 1.0)),
	triangle(vec3(3.5, 1.5, 2.5), vec3(3.5, 4.5, 2.5), vec3(2.5, 4.5, 2.5), vec3(0.0, 0.0, 1.0), vec3(0.1, 0.5, 1.0)),
	/* Front face triangles*/
	triangle(vec3(2.5, 1.5, 1.5), vec3(2.5, 4.5, 1.5), vec3(3.5, 4.5, 1.5), vec3(0.0, 0.0, -1.0), vec3(0.1, 0.5, 1.0)),
	triangle(vec3(2.5, 1.5, 1.5), vec3(3.5, 4.5, 1.5), vec3(3.5, 1.5, 1.5), vec3(0.0, 0.0, -1.0), vec3(0.1, 0.5, 1.0)),
	/* Right face triangles */
	triangle(vec3(3.5, 1.5, 1.5), vec3(3.5, 4.5, 1.5), vec3(3.5, 4.5, 2.5), vec3(1.0, 0.0, 0.0), vec3(0.1, 0.5, 1.0)),
	triangle(vec3(3.5, 1.5, 1.5), vec3(3.5, 4.5, 2.5), vec3(3.5, 1.5, 2.5), vec3(1.0, 0.0, 0.0), vec3(0.1, 0.5, 1.0)),
	/* Left face triangles */
	triangle(vec3(2.5, 1.5, 2.5), vec3(2.5, 4.5, 2.5), vec3(2.5, 4.5, 1.5), vec3(-1.0, 0.0, 0.0), vec3(0.1, 0.5, 1.0)),
	triangle(vec3(2.5, 1.5, 2.5), vec3(2.5, 4.5, 1.5), vec3(2.5, 1.5, 1.5), vec3(-1.0, 0.0, 0.0), vec3(0.1, 0.5, 1.0)),
	/* Top face triangles */
	triangle(vec3(2.5, 4.5, 1.5), vec3(2.5, 4.5, 2.5), vec3(3.5, 4.5, 2.5), vec3(0.0, 1.0, 0.0), vec3(0.1, 0.5, 1.0)),
	triangle(vec3(2.5, 4.5, 1.5), vec3(3.5, 4.5, 2.5), vec3(3.5, 4.5, 1.5), vec3(0.0, 1.0, 0.0), vec3(0.1, 0.5, 1.0)),
	/* Bottom face triangles */
	triangle(vec3(2.5, 1.5, 1.5), vec3(3.5, 1.5, 1.5), vec3(3.5, 1.5, 2.5), vec3(0.0, -1.0, 0.0), vec3(0.1, 0.5, 1.0)),
	triangle(vec3(2.5, 1.5, 1.5), vec3(3.5, 1.5, 2.5), vec3(2.5, 1.5, 2.5), vec3(0.0, -1.0, 0.0), vec3(0.1, 0.5, 1.0)),

	// Red box
	/* Cube Box */
	/* Back face triangles */
	triangle(vec3(-3.5, 1.0, 2.0), vec3(-1.5, 1.0, 2.0), vec3(-3.5, 2.0, 2.0), vec3(0.0, 0.0, -1.0), vec3(1.0, 0.25, 0.25)),
	triangle(vec3(-1.5, 1.0, 2.0), vec3(-1.5, 2.0, 2.0), vec3(-3.5, 2.0, 2.0), vec3(0.0, 0.0, -1.0), vec3(1.0, 0.25, 0.25)),
	/* Front face triangles*/
	triangle(vec3(-3.5, 1.0, 3.5), vec3(-3.5, 2.0, 3.5), vec3(-1.5, 2.0, 3.5), vec3(0.0, 0.0, 1.0), vec3(1.0, 0.25, 0.25)),
	triangle(vec3(-3.5, 1.0, 3.5), vec3(-1.5, 2.0, 3.5), vec3(-1.5, 1.0, 3.5), vec3(0.0, 0.0, 1.0), vec3(1.0, 0.25, 0.25)),
	/* Right face triangles */
	triangle(vec3(-1.5, 1.0, 3.5), vec3(-1.5, 2.0, 3.5), vec3(-1.5, 2.0, 2.0), vec3(1.0, 0.0, 0.0), vec3(1.0, 0.25, 0.25)),
	triangle(vec3(-1.5, 1.0, 3.5), vec3(-1.5, 2.0, 2.0), vec3(-1.5, 1.0, 2.0), vec3(1.0, 0.0, 0.0), vec3(1.0, 0.25, 0.25)),
	/* Left face triangles */
	triangle(vec3(-3.5, 1.0, 2.0), vec3(-3.5, 2.0, 2.0), vec3(-3.5, 2.0, 3.5), vec3(-1.0, 0.0, 0.0), vec3(1.0, 0.25, 0.25)),
	triangle(vec3(-3.5, 1.0, 2.0), vec3(-3.5, 2.0, 3.5), vec3(-3.5, 1.0, 3.5), vec3(-1.0, 0.0, 0.0), vec3(1.0, 0.25, 0.25)),
	/* Top face triangles */
	triangle(vec3(-3.5, 2.0, 3.5), vec3(-3.5, 2.0, 2.0), vec3(-1.5, 2.0, 2.0), vec3(0.0, 1.0, 0.0), vec3(1.0, 0.25, 0.25)),
	triangle(vec3(-3.5, 2.0, 3.5), vec3(-1.5, 2.0, 2.0), vec3(-1.5, 2.0, 3.5), vec3(0.0, 1.0, 0.0), vec3(1.0, 0.25, 0.25)),
	/* Bottom face triangles */
	triangle(vec3(-3.5, 1.0, 3.5), vec3(-1.5, 1.0, 3.5), vec3(-1.5, 1.0, 2.0), vec3(0.0, -1.0, 0.0), vec3(1.0, 0.25, 0.25)),
	triangle(vec3(-3.5, 1.0, 3.5), vec3(-1.5, 1.0, 2.0), vec3(-3.5, 1.0, 2.0), vec3(0.0, -1.0, 0.0), vec3(1.0, 0.25, 0.25)),

	// Green Box
	/* Cube Box */
	/* Back face triangles */
	triangle(vec3(-3.5, 1.0, -4.0), vec3(-1.5, 1.0, -4.0), vec3(-3.5, 2.0, -4.0), vec3(0.0, 0.0, -1.0), vec3(0.1, 1.0, 0.1)),
	triangle(vec3(-1.5, 1.0, -4.0), vec3(-1.5, 2.0, -4.0), vec3(-3.5, 2.0, -4.0), vec3(0.0, 0.0, -1.0), vec3(0.1, 1.0, 0.1)),
	/* Front face triangles*/
	triangle(vec3(-3.5, 1.0, -3.0), vec3(-3.5, 2.0, -3.0), vec3(-1.5, 2.0, -3.0), vec3(0.0, 0.0, 1.0), vec3(0.1, 1.0, 0.1)),
	triangle(vec3(-3.5, 1.0, -3.0), vec3(-1.5, 2.0, -3.0), vec3(-1.5, 1.0, -3.0), vec3(0.0, 0.0, 1.0), vec3(0.1, 1.0, 0.1)),
	/* Right face triangles */
	triangle(vec3(-1.5, 1.0, -3.0), vec3(-1.5, 2.0, -3.0), vec3(-1.5, 2.0, -4.0), vec3(1.0, 0.0, 0.0), vec3(0.1, 1.0, 0.1)),
	triangle(vec3(-1.5, 1.0, -3.0), vec3(-1.5, 2.0, -4.0), vec3(-1.5, 1.0, -4.0), vec3(1.0, 0.0, 0.0), vec3(0.1, 1.0, 0.1)),
	/* Left face triangles */
	triangle(vec3(-3.5, 1.0, -4.0), vec3(-3.5, 2.0, -4.0), vec3(-3.5, 2.0, -3.0), vec3(-1.0, 0.0, 0.0), vec3(0.1, 1.0, 0.1)),
	triangle(vec3(-3.5, 1.0, -4.0), vec3(-3.5, 2.0, -3.0), vec3(-3.5, 1.0, -3.0), vec3(-1.0, 0.0, 0.0), vec3(0.1, 1.0, 0.1)),
	/* Top face triangles */
	triangle(vec3(-3.5, 2.0, -3.0), vec3(-3.5, 2.0, -4.0), vec3(-1.5, 2.0, -4.0), vec3(0.0, 1.0, 0.0), vec3(0.1, 1.0, 0.1)),
	triangle(vec3(-3.5, 2.0, -3.0), vec3(-1.5, 2.0, -4.0), vec3(-1.5, 2.0, -3.0), vec3(0.0, 1.0, 0.0), vec3(0.1, 1.0, 0.1)),
	/* Bottom face triangles */
	triangle(vec3(-3.5, 1.0, -3.0), vec3(-1.5, 1.0, -3.0), vec3(-1.5, 1.0, -4.0), vec3(0.0, -1.0, 0.0), vec3(0.1, 1.0, 0.1)),
	triangle(vec3(-3.5, 1.0, -3.0), vec3(-1.5, 1.0, -4.0), vec3(-3.5, 1.0, -4.0), vec3(0.0, -1.0, 0.0), vec3(0.1, 1.0, 0.1))
);
...
vec4 trace(vec3 origin, vec3 dirEyeToTriangle)
{
	...
	if (intersectTriangles(origin, dirEyeToTriangle, eyeHitTriangle))
	{
		...
		for(int j = 0; j < MAX_LIGHTS; j++)
		{
			...
			vec3 reflection = addReflectionToPixColor(lights[j], dirEyeToTriangle, eyeHitTriangle, 10);
...
void main() 
{
	// position of pixel in 0 - 1 range
	vec2 pos = vec2(
